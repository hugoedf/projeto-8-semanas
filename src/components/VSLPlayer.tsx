...
const VSLPlayer = ({
  ...
  const startPlayback = async () => {
    ...
    try {
      ...
      playImpactSfx("spark");
      const response = await fetch(`https://kfddlytvdzqwopongnew.supabase.co/functions/v1/elevenlabs-vsl`, {
        method: "POST",
        headers: {
  ...
  return <div className="relative w-full max-w-3xl mx-auto">
      ...
      <div className="relative aspect-video bg-black rounded-lg overflow-hidden shadow-[0_0_60px_rgba(0,0,0,0.8)]">
        ...
        {!hasStarted && <>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center z-0 p-6">
              <p className="text-white/90 text-xl sm:text-2xl md:text-3xl font-display font-black uppercase tracking-wider mb-3 text-center" style={{
        ...
        {!hasStarted && <div className="absolute inset-0 flex items-center justify-center z-10">
            <button onClick={startPlayback} disabled={phase === "loading"} className="group relative w-16 h-16 sm:w-20 sm:h-20 rounded-full border-2 border-white/30 hover:border-accent/80 bg-black/40 hover:bg-accent/20 backdrop-blur-sm flex items-center justify-center transition-all duration-500 hover:scale-110 disabled:opacity-70 disabled:hover:scale-100" style={{
          boxShadow: "0 0 40px rgba(0,0,0,0.5), inset 0 0 20px rgba(255,255,255,0.05)"
              ...
              <span className={`absolute inset-0 rounded-full border border-white/20 opacity-30 ${phase === "loading" ? "" : "animate-ping"}`} />
            </button>
          </div>}

        {/* Thumbnail text - Minimal */}
        {!hasStarted && <div className="absolute bottom-8 left-0 right-0 flex justify-center z-0">
      ...
      {phase === "ended" && <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 text-accent animate-bounce">
