...
const VSLPlayer = ({ onVideoEnd, onProgress }: VSLPlayerProps) => {
  ...
  const togglePlayPause = async () => {
    ...
    if (!video.paused && !video.ended) {
      ...
      document.querySelectorAll<HTMLMediaElement>('audio, video').forEach(el => {
        try {
          if (el !== video) { el.pause(); el.muted = true; }
        } catch {}
      });
      return;
    }
  ...
  useEffect(() => {
    ...
    const handlePause = () => { setIsPlaying(false); video.muted = true; setIsMuted(true); };
  ...
  const handleTimeUpdate = () => {
    ...
    percentMilestones.forEach(milestone => {
      if (currentProgress >= milestone && !milestonesRef.current.has(milestone)) {
        ...
        if (milestone === 25) {
          window.dispatchEvent(new CustomEvent('vsl-progress-25', { detail: { progress: currentProgress, visitorId: visitorData?.visitorId } }));
        }
      }
    });
  };
  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-3xl mx-auto group" onMouseMove={handleMouseMove} onMouseLeave={() => isPlaying && setShowControls(false)}>
      <div className="relative aspect-video bg-black rounded-lg overflow-hidden shadow-[0_0_60px_rgba(0,0,0,0.8)]">

        {!hasStarted && (
          <>
            <img src={vslThumbnail} alt="VSL Thumbnail" className="absolute inset-0 w-full h-full object-cover" width={1920} height={1080} fetchPriority="high" decoding="async" />
            ...
            <div className="absolute bottom-4 sm:bottom-8 left-0 right-0 text-center z-20 px-4">
              ...
              <p className="text-white/70 text-[10px] sm:text-xs mt-2 flex items-center justify-center gap-1.5 font-medium">
                <Volume2 className="w-3.5 h-3.5" /> <span>Melhor com som</span>
              </p>
            </div>
          </>
        )}
        ...
        {hasStarted && (
          <div className="absolute inset-0 cursor-pointer" onClick={() => setShowControls(!showControls)}>
            {!isPlaying && (
